(function (global, factory) {
  if (typeof define === "function" && define.amd) {
    ...
    global.ScrollAnimator = mod.exports;
  }
  ...
  class ScrollAnimator {
    ...
    _calcScrollProgress() {
      if (!this.isStatic) {
        return (_utils.scroll.y() - this.offsetTop) / (_utils.dimensions.y() * this.heightFactor);
      } else {
        return (_utils.scroll.y() - this.offsetTop) / _utils.dimensions.y();
      }
    }

    _reverseProgress(progress) {
      if (!this.isStatic) {
        return progress * (_utils.dimensions.y() * this.heightFactor) + this.offsetTop;
      } else {
        return progress * _utils.dimensions.y() + this.offsetTop;
      }
    }
        ...
        if (!this.isStatic) {
          if (this.activeSlide < newSlideIndex) {
            for (let i = 0; i < newSlideIndex; i++) {
              window.scrollTo(0, _utils.dimensions.y() * this.heightFactor * i);
              this._render();
              window.scrollTo(0, _utils.dimensions.y() * this.heightFactor * (i + 0.25));
              this._render();
              window.scrollTo(0, _utils.dimensions.y() * this.heightFactor * (i + 0.50));
              this._render();
              window.scrollTo(0, _utils.dimensions.y() * this.heightFactor * (i + 0.75));
              this._render();
              window.scrollTo(0, _utils.dimensions.y() * this.heightFactor * (i + 0.999999));
              this._render();
              window.scrollTo(0, _utils.dimensions.y() * this.heightFactor * (i + 1));
              this._render();
            }
            window.scrollTo(0, _utils.dimensions.y() * this.heightFactor * (newSlideIndex * 1.1));
            this._render();
          } else {
            for (let i = this.slides.length - 1; i > newSlideIndex; i--) {
              ...
              window.scrollTo(0, _utils.dimensions.y() * this.heightFactor * (i + 0.25));
              this._render();
              window.scrollTo(0, _utils.dimensions.y() * this.heightFactor * (i + 0.0000001));
              this._render();
              window.scrollTo(0, _utils.dimensions.y() * this.heightFactor * (i + 0));
              this._render();
            }
            window.scrollTo(0, _utils.dimensions.y() * this.heightFactor * (newSlideIndex * 1.1));
            this._render();
  ...
  exports.default = ScrollAnimator;
});